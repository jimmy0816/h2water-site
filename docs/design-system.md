...
| # | 原則 | 說明 |
|---|------|------|
| 1 | **能用文字就不用 icon** | 日系設計靠 typography 建立層次。時鐘→`12 min`、書本→`引用 50 篇` |
| 2 | **僅存 icon 規格** | `strokeWidth: 1.2–1.3px`、`strokeLinecap: "square"`、單色 `currentColor` |
| 3 | **品牌圖示採紋樣化（Mon 風格）** | 圓內構圖、幾何抽象、三圓交疊（HHO）、1px 描邊、opacity 0.25 |
| 4 | **⛔ 禁止 Emoji 作為 UI 元素** | 跨平台不一致、彩度過高 |
| 5 | **⛔ 禁止 Feather / Lucide 預設風格** | 線條太粗（2px）、端點太圓（round cap） |
| 6 | **borderRadius 收緊** | 卡片 6px、按鈕/badge 3px |
| 7 | **全站最多 2 種 icon** | 分子紋（裝飾）+ 箭頭（導航），其餘一律用文字 |

### 證據標示場景規則

| 場景 | 圖示 | 底色 | 說明 |
|------|------|------|------|
| 標籤場景（卡片、sidebar、hero） | 幾何紋 | ✓ 底色色塊 | 全站預設 |
| 融入場景（行內文字、tooltip、email） | 幾何紋（行內）或無 | ✗ 無底色 | 不打斷閱讀 |
| 大尺寸/教學場景 | 印章風（圓形+漢字） | 視場景 | 備用方案 |
| 純文字環境（email、RSS） | 無圖示 | 無 | fallback |

### 字重下限原則

- 內文最低字重：**400**（⛔ 禁止 300）
- meta / 輔助資訊最低字重：**400**（⛔ 禁止 300）
- **300 僅保留給 Display 大標題（≥32px）的裝飾用途**
